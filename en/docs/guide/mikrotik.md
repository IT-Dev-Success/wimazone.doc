...
This guide covers the full installation of WimaZone Billing on a MikroTik router using container mode.

## Supported routers (project reference)

| Model | Architecture | RAM | Compatibility |
|---|---|---:|---|
| L009UiGS-2HaxD-IN | ARM 32-bit | 512 MB | Compatible |
| L009UiGS-RM | ARM 32-bit | 512 MB | Compatible |
| hAP ac3 | ARM 32-bit | 256 MB | Compatible |
...
## 3) Create the billing container VETH interface
...
## 4) Add outbound NAT for containers
...
## 6) Create persistent SQLite storage

```routeros
/container/mounts/add name=billing-db src=usb1/billing-data dst=/data
```

Expected persistent database path:
...
> `GITHUB_PRIVATE_ACCESS_TOKEN` is provided by ITDevSuccess.

## 8) Create the Wima Zone container
...
## 11) Recommended Walled Garden rules
...
- `GIT_OFFLINE_FALLBACK=true`: container starts with local code when GitHub is unavailable.
- `DB_DATABASE=/data/database.sqlite`: DB persists across container recreation.
- `LARAVEL_AUTO_STORAGE_LINK=false`: prevents boot blocking on some mount setups.
- Prepare frontend assets in advance (avoid heavy frontend builds on MikroTik).

## 14) Quick troubleshooting

If you see `SIGKILL`, `502`, or `max_children`:

1. Check host memory / OOM conditions.
2. Verify API credentials and port (`8728`/`8729`).
3. Ensure hotspot files are synchronized.
4. Keep `MIKROTIK_BOOT_HOTSPOT_SYNC_PROCESS_NOW=false`.
5. Keep `/hotspot/status` timeout/cache/cooldown values configured.
